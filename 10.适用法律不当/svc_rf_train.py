...
"""
use TFIDF + LinearSVC/RandomForest to predict law
"""
...
class Model(object):
    ...
    def train(self,train_fname):
        ...
        facts, article_label = self.load_data(train_fname)  # all in `pandas.Series` form
    ...
    def load_data(self, train_fname):
        ...
        with open(train_fname, 'r') as f:
            ...
            while line:
                ...
                article = util.get_label(line_dict, "law") #0-182
        ...
        if util.DUMP:
            self.dump_processed_data_to_file(facts,article_label)

        return facts,article_label

    def dump_processed_data_to_file(self, facts, article_label):
        """ dump processed data to `.pkl` file """
        data = [facts, article_label]
